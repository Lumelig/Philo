...
static bool philo_died(t_philo *philo)
{
    ...
    if (get_bool(&philo->philo_mtx, &philo->full))
        return (false);
    elapse = gettime(MILLISECOND) - get_long(&philo->philo_mtx, &philo->last_meal);
...
void   *monitor_dinner(void *data)
{
    ...
    table = (t_table *)data;
    while (!all_threads_running(&table->table_mtx, &table->threads_running_nbr, table->philosophers))
        ;
